            ...
            <div class="w-full p-5">
                <div v-if="errors.name" class="mb-5 bg-red-100 border border-red-500 p-5 rounded-xl">
                    <ul class="flex flex-col gap-5 text-red-500 text-sm">
                        <li>لطفا همه مقادیر را بررسی کنید</li>
                    </ul>
                </div>
                    ...
                    <div class="flex flex-col gap-5">
                        <div class="w-full">
                            <div class="flex-1 flex dark">
                                <input v-model="expense.name" type="text" class="rounded-none rounded-lg bg-gray-50 border text-gray-900 focus:ring-blue-500 focus:border-blue-500 block flex-1 min-w-0 w-full text-sm border-gray-300 p-2.5 duration-150" placeholder="نام هزینه">
                            </div>
                        </div>
                        <div class="w-full">
                            <div class="flex-1 flex dark flex-col gap-5">
                                <span class="text-sm">خرید کننده</span>
                                <select v-model="expense.people_id" class="w-full px-3 py-2 rounded-xl">
                        ...
                        <div class="w-full">
                            <div class="flex-1 flex dark flex-col gap-5">
                                <span class="text-sm">گروه مربوط</span>
                                <select v-model="expense.group_id" class="w-full px-3 py-2 rounded-xl">
                                    <option selected value="">انتخاب کنید</option>
                        ...
                        <div class="w-full">
                            <div class="flex-1 flex dark">
                                <input v-model="expense.total_payment" type="number" class="rounded-none rounded-lg bg-gray-50 border text-gray-900 focus:ring-blue-500 focus:border-blue-500 block flex-1 min-w-0 w-full text-sm border-gray-300 p-2.5 duration-150" placeholder="میزان هزینه">
                            </div>
                        </div>
                        <div class="w-full">
                            <div class="flex-1 flex dark">
                                <input v-model="expense.counting_order" type="number" class="rounded-none rounded-lg bg-gray-50 border text-gray-900 focus:ring-blue-500 focus:border-blue-500 block flex-1 min-w-0 w-full text-sm border-gray-300 p-2.5 duration-150" placeholder="نوبت خرید">
                            </div>
                        </div>
                        <div class="w-full">
...
const emit = defineEmits(['refreshExpense'])
...
const createExpense = () => {
    ...
    let data = {
        ...
        people_id: expense.value.people_id,
        group_id: expense.value.group_id,
        counting_order: expense.value.counting_order,
        total_payment: expense.value.total_payment,
    }
    loading.value = true;
    store.dispatch('createExpense', data)
        .then(res => {
            loading.value = false;
            if(res)
            {
...
onMounted(() => {
    ...
    store.dispatch('allGroups')
        .then(() => {
            groups.value = store.state.group.groups;
        })
})
</script>